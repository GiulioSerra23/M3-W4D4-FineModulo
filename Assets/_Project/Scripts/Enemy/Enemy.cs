...
public class Enemy : Creature
{
    ...
    private void EnemyMovement()
    {
        ...
        _animHandler.SetDirectionAndSetMoving(direction);
    }
    ...
    {
        if (!collision.collider.TryGetComponent<PlayerController>(out var player)) return;

        if (!player.TryGetComponent<LifeController>(out var lifeController)) return;       
